...
of LOD method that is used (fixed LOD or Negative Control LOD).*
...
for LOD to be calculated. More information on `clean_npx()` can be found in the
[Olink Analyze Overview tutorial](https://CRAN.R-project.org/package=OlinkAnalyze).*
...
## Generate check log on cleaned data with LOD
check_log_df_npx_fixed_lod_clean <- OlinkAnalyze::check_npx(
...
check_log_df_npx_fixed_lod <- check_log_df_npx_fixed_lod_clean
rm(
  df_npx_fixed_lod_clean,
  check_log_df_npx_fixed_lod_clean
)
```
...
## Generate check log on cleaned data
check_log_df_npx_both_lod_clean <- OlinkAnalyze::check_npx(
...
check_log_df_npx_both_lod <- check_log_df_npx_both_lod_clean
rm(
  df_npx_both_lod_clean,
  check_log_df_npx_both_lod_clean
)
