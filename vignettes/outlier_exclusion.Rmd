...
presence of outliers. When performing statistical analyze it is important to
...
NPX distribution plots generated by `olink_dist_plot` consist of box and whisker
plots of NPX distribution for each sample. These boxplots can be used to
...
outlier_data |>
  ...
  OlinkAnalyze::olink_dist_plot(check_log = check_log_npx_data1_clean)
```
...
```{r, eval = FALSE}
outlier_data |>
  OlinkAnalyze::olink_qc_plot(check_log = check_log_npx_data1_clean)
```
