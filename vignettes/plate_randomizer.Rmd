...
The number of samples on each plate could be specified via the
`available.spots` argument. For example, the following code will lead to 48
samples on the first and the second plate, and 42 samples on the third plates.
...
randomized_manifest_SampleNO. <- OlinkAnalyze::olink_plate_randomizer(
  Manifest = OlinkAnalyze::manifest,
  SubjectColumn = "SubjectID",
...
fcap3 <- "Figure 3. Randomized samples in a 96-well plate format, with the number of samples per plate pre-defined."
fcap4 <- "Figure 4. Randomized samples in 96 well plate format, colored by Studies."
...
OlinkAnalyze::olink_displayPlateLayout(
  data = randomized_manifest_SampleNO.,
  fill.color = "SubjectID",
  include.label = FALSE
)
```
