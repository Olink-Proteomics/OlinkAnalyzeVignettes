...
randomized_manifest_v1 <- OlinkAnalyze::olink_plate_randomizer(
...
However, if there are to many samples per subject (>8), complete randomization
is recommended. The number of samples on each plate could be specified via the
`available.spots` argument. For example, the following code will lead to 78
samples on the first plate, and 30 samples on the second and the third plates.
The number of iterations for fitting subjects on the same plate can be set by
...
The plate size can be either directly set to 48 or 96 using the `PlateSize`
...
randomized_manifest_v2 <- OlinkAnalyze::olink_plate_randomizer(
  Manifest = OlinkAnalyze::manifest,
  Product = "Explore HT",
...
argument can be used to group samples by study and randomized the samples within
the study. In the example below, "Site" is used as a surrogate for study. After
randomization the sites are kept together across the plates.

```{r}
randomized_manifest_c <- OlinkAnalyze::olink_plate_randomizer(
  Manifest = OlinkAnalyze::manifest,
  study = "Site"
)

OlinkAnalyze::olink_displayPlateLayout(
  data = randomized_manifest_c,
  fill.color = "Site"
)
```
...
fcap1 <- paste("Figure 1. Randomized samples in 96 well plate format, colored",
               "by Subject ID.")
fcap2 <- paste("Figure 2. Randomized samples in 96 well plate format with",
               "labeled wells.")
...
visualized via a bar chart using function `olink_displayPlateDistributions`. By
...
We could also check the distribution of other variables. For examples, the
...
In the case of the randomization is not performed well, the randomization could
be performed again with different seed number and iterations number. 
...
Manifest with additional columns:
...
`write_xlsx` function from Package `writexl`.

```{r, eval = FALSE}
writexl::write_xlsx(
  x = randomized_manifest_v1,
