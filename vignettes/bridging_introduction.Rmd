...
dplyr::tibble(
  "Olink Product" = c(
    ...
    paste("Explore 384: Cardiometabolic II, Inflammation II, Neurology II,",
          "and Oncology II"),
    "Explore 3072",
    "Explore HT",
    "Reveal",
  ...
  kableExtra::kbl(
    ...
    digits = 2L,
    caption =
      paste("Table 1. Recommended number of bridge samples for Olink products.")
  ) |>
...
datasets contain control samples with duplicated SampleIDs across over datasets.
Because downstream functions does not allow duplicates we will rename the
control samples to have unique SampleIDs by appending a unique suffix to the
control sample IDs in each dataset. This process is shown in the code below.
...
2 shows a large overlap between npx_data1 and npx_data2, however there are still
some differences as indicated by the blue and red areas on the edge of the
distribution.
...
of each entry corresponds to the same sample. For example, if a sample had the
